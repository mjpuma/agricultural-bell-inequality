        ...
        try:
            ...
            if len(crisis_data) > 20:
                ...
                if s1_values:
                    ...
                    crisis_s1 = s1_series[crisis_mask].values
                    crisis_violations = pd.Series(s1_violations, index=data.index[-len(s1_violations):])[crisis_mask].values
                else:
                    crisis_s1 = []
                    crisis_violations = []
                
                # Create crisis zoom plot
                fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 10))
                fig.suptitle(f'{crisis_name} Zoom: {asset1} vs {asset2}', fontsize=14, fontweight='bold')
                
                # Price plot
                ax1.plot(crisis_data.index, crisis_data[asset1], 'b-', linewidth=1.5, label=f'{asset1}', alpha=0.8)
                ax1.plot(crisis_data.index, crisis_data[asset2], 'orange', linewidth=1.5, label=f'{asset2}', alpha=0.8)
                ax1.set_ylabel('Price')
                ax1.set_title(f'Stock Prices During {crisis_name}')
                ax1.legend()
                ax1.grid(True, alpha=0.3)
                
                # S1 Bell inequality plot
                if crisis_s1:
                    ax2.plot(crisis_data.index[-len(crisis_s1):], crisis_s1, 'b-', linewidth=1.5, label='|S1|', alpha=0.8)
                    ...
                    violations_array = np.array(crisis_violations)
                    if violations_array.any():
                        violation_indices = crisis_data.index[-len(crisis_s1):][violations_array]
                        violation_values = np.array(crisis_s1)[violations_array]
                        ax2.scatter(violation_indices, violation_values, color='red', s=30, alpha=0.7, 
                                   label=f'Violations ({violations_array.sum()})', zorder=5)
                
                ax2.set_ylabel('|S1|')
                ax2.set_title(f'S1 Bell Inequality During {crisis_name}')
                ax2.legend()
                ...
                # Save crisis zoom plot
                filename = f"{asset1}_{asset2}_{crisis_name}_zoom.png"
                results_mgr.save_figure(fig, filename, dpi=300)
                plt.close(fig)
                
                print(f"   📊 Crisis zoom plot saved: {filename}")
                
        except Exception as e:
            print(f"   ⚠️  Could not create crisis zoom plot for {crisis_name}: {e}")
...
def create_enhanced_figure(asset1, asset2, data, returns, rolling_corr, rolling_vol1, rolling_vol2, 
    ...
    if data_length > 1000:  # More than ~4 years of daily data
        ...
        if mandelbrot_series and 'cross_hurst' in mandelbrot_series:
    ...
    # S1 Bell Inequality plot
    ax1 = axes[0, 0]
    if plot_s1_values and plot_s1_violations:
    ...
    ax2 = axes[0, 1]
    if mandelbrot_series_resampled and 'cross_hurst' in mandelbrot_series_resampled:
        ...
        ax2.set_ylabel('Cross-Hurst Exponent')
        ax2.set_title('Mandelbrot Cross-Hurst Time Series')
    ...
    ax3.set_title('20-day Rolling Correlation (RETURNS)')
    ...
    if mandelbrot_series_resampled and len(mandelbrot_series_resampled) > 1:
        mandelbrot_time_index = plot_data.index[-len(mandelbrot_series_resampled['cross_hurst']):]
        
        # Cross-correlation decay
        ax5 = axes[2, 0]
        if any(not np.isnan(x) for x in mandelbrot_series_resampled['cross_correlation_decay']):
        ...
        # Lead-lag strength
        ax6 = axes[2, 1]
        if any(not np.isnan(x) for x in mandelbrot_series_resampled['lead_lag_strength']):
            ax6.plot(mandelbrot_time_index, mandelbrot_series_resampled['lead_lag_strength'], 
                    'red', linewidth=1.5, label='Lead-Lag Strength', alpha=0.8)
            ax6.set_ylabel('Lead-Lag Strength')
            ax6.set_title('Mandelbrot Lead-Lag Strength')
...
def create_enhanced_correlation_table(returns1, returns2, rolling_corr, s1_values, mandelbrot_series):
    ...
    if mandelbrot_series and s1_values and len(s1_values) > 10:
        s1_series = pd.Series(s1_values)
    ...
    table_data = [
        ...
        ('Average S1 Value', f'{np.mean(s1_values):.6f}' if s1_values else 'N/A', 'N/A'),
        ('S1 Standard Deviation', f'{np.std(s1_values):.6f}' if s1_values else 'N/A', 'N/A')
    ]
            ...
            try:
                ...
                if crisis_data is not None and not crisis_data.empty and len(crisis_data) > 20:
                    ...
                    if len(close_data) > 20:
                        ...
                        print(f"   ❌ {pair_name}: Insufficient crisis data")
                else:
                    print(f"   ❌ {pair_name}: No crisis data available")
                    
            except Exception as e:
                print(f"   ❌ {pair_name}: Crisis analysis failed - {e}")
